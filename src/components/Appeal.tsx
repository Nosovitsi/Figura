...
const Appeals: React.FC = () => {
  ...
  const getFieldValue = (appeal: Appeal, fieldName: string): string => {
    const possibleFields: FieldMapping = {
      ...
      'Описание проблемы': ['Problem description', 'Problem_description', 'problemDescription', 'description', 'problem'],
      ...
      '№ договора тех поддержки': ['supp_number', 'suppNumber', 'supportNumber'],
                    ...
                    <td key={columnName}>
                      {columnName === 'Прикрепленные файлы' ? (
                        getFieldValue(appeal, columnName) && (
                          <a href={getFieldValue(appeal, columnName)} target="_blank" rel="noopener noreferrer">
                            📎 Скачать
                          </a>
                        )
                      ) : (
